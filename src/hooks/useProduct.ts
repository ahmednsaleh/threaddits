...
export function useProductSubreddits(productId: string | undefined) {
  return useQuery({
    ...
    queryFn: async (): Promise<SubredditStatus[]> => {
      ...
      const [{ data: tracked, error }, { data: product }] = await Promise.all([
        supabase
          .from("product_subreddit_status")
          .select(
            "id, subreddit, status, total_leads_found, avg_intent_score, origin",
          )
      ...
      const trackedSet = new Set(
        (tracked || []).map((t: any) => t.subreddit?.toLowerCase()),
      );
      const fromProduct: SubredditStatus[] = (product?.subreddits || [])
        .filter((sub: string) => !trackedSet.has(sub.toLowerCase()))
        .map((sub: string) => ({
          id: `cfg-${sub}`,
          subreddit: sub,
          status: "active",
          total_leads_found: 0,
          avg_intent_score: null,
          origin: "user_added",
        }));

      return [...(tracked || []), ...fromProduct];
    },
    enabled: !!productId,
  });
}
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<KeywordPerformance[]> => {
      ...
      const { data, error } = await supabase
        ...
        .select(
          "id, keyword, status, leads_found, impressions, conversion_rate",
        )
        .eq("product_id", productId)
...
export function useToggleProductStatus() {
  ...
  return useMutation({
    ...
    onSuccess: (_, { newStatus }) => {
      ...
      toast.success(
        newStatus === "active" ? "Hunting Resumed" : "Hunting Paused",
      );
    },
    onError: (error) => {
