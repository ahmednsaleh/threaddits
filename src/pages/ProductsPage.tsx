...
}> = ({ product, onToggle, onDelete }) => {
  const navigate = useNavigate();
  const isActive = product.status === "active";
  const painPoints = product.pain_points_solved
    ? product.pain_points_solved
        .split(",")
        .map((s) => s.trim())
        .filter(Boolean)
    : [];
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {painPoints.slice(0, 2).map((pt, i) => (
                <span
                  key={i}
                  className="px-2 py-1 rounded-md bg-slate-100 text-slate-600 border border-slate-200 text-xs font-medium"
                >
                  {pt}
                </span>
              ))}
      ...
      <div className="mt-auto mb-8 border-t border-slate-100 pt-6">
        ...
        <div className="flex items-center gap-1.5 mt-2 text-slate-400">
          ...
          <span className="text-xs font-mono">
            {product.last_run_at
              ? `Last sweep: ${formatTimeAgo(product.last_run_at)}`
              : "No sweeps yet"}
          </span>
        </div>
      </div>
...
const EmptySlot: React.FC<{ isLocked: boolean; onAction: () => void }> = ({
  isLocked,
  onAction,
}) => {
  return (
    <div
      onClick={onAction}
      className={cn(
        "border-2 border-dashed border-slate-300 rounded-2xl p-8 flex flex-col items-center justify-center text-center transition-all group min-h-[400px]",
        isLocked
          ? "bg-slate-50/30"
          : "bg-slate-50/50 hover:bg-slate-100 cursor-pointer hover:border-slate-400",
      )}
    >
      <div
        className={cn(
          "w-16 h-16 rounded-2xl flex items-center justify-center mb-6 border bg-white",
          isLocked
            ? "text-slate-300"
            : "text-slate-400 group-hover:text-[#C2410C] group-hover:border-[#C2410C]/20 shadow-sm",
        )}
      >
      ...
      >
        {isLocked ? `Unlock Slot` : "Add New Product"}
      </h3>
      <p className="text-slate-400 text-base max-w-[220px] leading-relaxed">
        {isLocked
          ? "Upgrade to Pro to hunt for another product."
          : "Configure a new persona to start hunting in different subreddits."}
      </p>
      {isLocked && (
        <Button
          variant="link"
          ...
          className="mt-6 text-[#C2410C] font-bold text-sm uppercase tracking-widest"
        >
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div>
            ...
            <div className="flex items-center gap-3 mt-3">
              <div className="px-3 py-1 rounded-full bg-slate-100 text-slate-500 text-[10px] font-bold uppercase tracking-widest border border-slate-200">
                Plan: {subscriptionTier}
              </div>
              <p className="text-slate-400 text-sm font-medium">
                Manage target personas and value propositions.
              </p>
            </div>
          </div>
          {isPro && (
            <Button
              ...
              className="bg-[#C2410C] text-white h-12 px-8 rounded-full font-bold shadow-lg hover:-translate-y-0.5 transition-all"
            >
          ...
          {Array.from({
            ...
            return (
              <EmptySlot
                ...
                isLocked={isLocked}
                onAction={
                  isLocked ? () => navigate("/settings") : handleAddProduct
                }
              />
            );
          })}
