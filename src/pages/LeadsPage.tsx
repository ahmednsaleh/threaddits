...
export default function LeadsPage({
  ...
  const [draftData, setDraftData] = React.useState<
    Record<string, { id: string; draft_text: string } | null>
  >({});
  ...
  useEffect(() => {
    ...
    supabase
      .from("lead_drafts")
      .select("id, lead_id, draft_text")
      .in("lead_id", leadIds)
      .order("created_at", { ascending: false })
      .then(({ data }) => {
        if (!data) return;
        // Keep only the most recent draft per lead
        const map: Record<string, { id: string; draft_text: string }> = {};
        for (const row of data) {
          if (!map[row.lead_id])
            map[row.lead_id] = { id: row.id, draft_text: row.draft_text };
        }
        setDraftData(map);
      });
  ...
  const filteredLeads = useMemo(() => {
    return leads.filter((lead) => {
      ...
      if (
        activeFilter === "warm" &&
        (lead.intent_score < 5 || lead.intent_score >= 8)
      )
        return false;
  ...
  return (
    <main
      className={cn(
        ...
        isShowcase
          ? "p-6 overflow-hidden"
          : "p-4 md:gap-8 md:p-10 overflow-y-auto",
      )}
    >
      <div
        className={cn(
      ...
      >
          ...
          <div className="relative">
            ...
            >
              ...
              >
                {isLoadingProducts
                  ? "Loading..."
                  : activeProduct?.product_name || "Select Product"}
                <ChevronDown
                  className={cn(
              ...
              {activeProduct?.status === "active" && (
                <div className="bg-emerald-50 text-emerald-600 px-2 py-0.5 rounded-full text-xs font-medium border border-emerald-100 flex items-center gap-1.5 shadow-sm h-7 mt-1">
                  ...
                  <span className="font-bold uppercase tracking-widest text-[10px] pt-0.5">
                    Live
                  </span>
                </div>
              )}
        ...
        <div className="bg-white border border-slate-200 rounded-2xl h-24 flex items-stretch shadow-sm overflow-hidden shrink-0 relative z-0">
          <button
            ...
            className={cn(
              "flex-1 flex flex-col items-center justify-center border-r border-slate-100 px-4 transition-all focus:outline-none",
              activeFilter === "all"
                ? "bg-slate-50 border-b-2 border-b-[#C2410C]"
                : "hover:bg-slate-50/50",
            )}
          >
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em] mb-2 flex items-center gap-1.5">
              <Layout className="w-3 h-3" /> Total Leads
            </span>
          ...
          <button
            ...
            className={cn(
              "flex-1 flex flex-col items-center justify-center px-4 transition-all focus:outline-none",
              activeFilter === "hot"
                ? "bg-slate-50 border-b-2 border-b-[#C2410C]"
                : "hover:bg-slate-50/50",
            )}
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="relative">
              <Button
                ...
                className="rounded-full border-slate-200 h-11 px-6 flex items-center gap-2"
              >
                <Clock className="w-3.5 h-3.5" /> {timeFilter}{" "}
                <ChevronDown className="w-3.5 h-3.5" />
              </Button>
              {isTimeMenuOpen && (
                <>
        ...
        >
          {isLoading && !isShowcase ? (
            ...
            <>
              {visibleLeads
                .slice(0, isShowcase ? 2 : undefined)
                .map((lead, index) => (
                  <LeadCard
                    key={lead.id}
                    id={lead.id}
                    ...
                    competitors_mentioned={
                      ...
                      lead.competitors_mentioned ||
                      "None"
                    }
                    buying_stage={
                      lead.buying_stage_detail ||
                      (lead.is_solution_seeking
                        ? "Solution Seeking"
                        : "Researching")
                    }
                    sentiment={
              ...
              {!isShowcase && lockedLeadCount > 0 && (
                <div className="bg-gradient-to-br from-[#2C3E50] to-[#1E293B] rounded-2xl p-10 text-white text-center shadow-2xl">
                  ...
                  <h3 className="text-2xl font-bold mb-2">
                    {lockedLeadCount} more high-intent lead
                    {lockedLeadCount === 1 ? "" : "s"} found
                  </h3>
                  <p className="text-slate-300 text-sm leading-relaxed mb-8 max-w-md mx-auto">
                    Your Reddit scan already found {filteredLeads.length} leads
                    for{" "}
                    <span className="font-bold text-white">
                      {activeProduct?.product_name}
                    </span>
                    . Subscribe to unlock all of them and get hourly fresh
                    leads.
                  </p>
            ...
            <div className="py-20 text-center bg-white border border-dashed border-slate-200 rounded-2xl">
              <p className="text-slate-400 font-bold uppercase tracking-widest text-sm font-mono">
                {!activeProductId
                  ? "Please select a product to view leads."
