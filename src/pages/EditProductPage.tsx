...
const TierBadge = ({ tier }: { tier: string }) => {
  ...
  return (
    <span
      className={cn(
        "px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest border font-mono whitespace-nowrap",
...
export default function EditProductPage() {
  ...
  const { data: subreddits = [], isLoading: isLoadingSubreddits } =
    useProductSubreddits(id);
  const { data: keywords = [], isLoading: isLoadingKeywords } =
  ...
  const handleDiscoverSubreddits = async () => {
    if (!id) return;
    toast.info("Discovering adjacent subreddits...", {
          ...
          <div className="flex-1 flex flex-col gap-8 min-w-0">
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-slate-500 mb-1">
                    What is your product, and how does it help solve these
                    customer problems or alleviate their frustrations?
                  </p>
                  <textarea
            ...
            <div className="bg-white rounded-2xl border border-slate-200 p-6 md:p-8 shadow-sm">
              ...
              {isLoadingSubreddits ? (
                ...
                <div className="text-center py-8 text-slate-400 text-sm">
                  No subreddits configured. Add one or use "Find Adjacent" to
                ...
                <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
                  {subreddits.map((sub) => {
                    ...
                    return (
                      <div
                        key={sub.id}
                        className="group relative flex items-center justify-between bg-white border border-slate-200 rounded-lg px-4 py-3 hover:border-[#C2410C]/30 hover:shadow-sm transition-all cursor-default select-none"
                      >
            ...
            <div className="bg-white rounded-2xl border border-slate-200 p-6 md:p-8 shadow-sm flex-1 flex flex-col">
              ...
              <p className="text-xs text-slate-400 mt-1 font-medium">
                Keywords with 0 impressions haven't been included in any crawl
                run yet.
              </p>
