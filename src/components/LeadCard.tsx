...
const renderHighlightedSummary = (text: string, competitors: string) => {
  if (
    !competitors ||
    competitors === "No direct competitors mentioned" ||
    competitors === "None"
  )
    return text;
  ...
  return parts.map((part, i) => {
    const isMatch = compList.some(
      (c) => c.toLowerCase() === part.toLowerCase(),
    );
    return isMatch ? (
      <span
        key={i}
        className="bg-yellow-100 text-slate-900 font-bold px-0.5 rounded box-decoration-clone"
      >
        {part}
      </span>
    ) : (
...
export const LeadCard: React.FC<LeadCardProps> = ({
  ...
  const cleanSubreddit = source_subreddit.startsWith("r/")
    ? source_subreddit
    : `r/${source_subreddit}`;
    ...
    >
      ...
      <div className="mb-6">
        ...
        <div className="flex flex-wrap items-center gap-2 text-xs font-mono text-slate-500">
          ...
          <span className="font-bold bg-slate-100 px-1.5 py-0.5 rounded text-slate-600">
            {cleanSubreddit}
          </span>
          <span className="text-slate-300">•</span>
      ...
      <div className="bg-gradient-to-r from-[#C2410C]/10 to-white rounded-r-lg border-l-4 border-[#C2410C] p-5 mb-6 shadow-sm">
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="text-xs font-bold text-[#C2410C] uppercase tracking-widest">
            Match Intelligence for {product_name}
          </span>
        </div>
          ...
          >
            <div
              className={cn(
                ...
                funnelStatuses.find((s) => s.id === currentStatus)?.color ||
                  "bg-blue-500",
              )}
            />
          ...
          {isStatusMenuOpen && (
            <div className="absolute bottom-full left-0 mb-2 w-40 bg-white border border-slate-200 rounded-lg shadow-xl z-20 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
              {funnelStatuses.map((statusItem) => (
                ...
                >
                  <div
                    className={cn("w-2 h-2 rounded-full", statusItem.color)}
                  />
                  {statusItem.label}
                </button>
              ))}
